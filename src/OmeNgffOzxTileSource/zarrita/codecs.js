...
function create_default_registry() {
  return new Map()
    ...
    .set("jpeg", () => import("./codecs/jpeg.js").then((m) => m.JpegCodec));
}
...
async function load_codecs(chunk_meta) {
  let promises = chunk_meta.codecs.map(async (meta) => {
    ...
    if (!Codec) {
      console.error(`Unknown codec: ${meta.name}. Available codecs:`, Array.from(registry.keys()));
  ...
  for await (let { Codec, meta } of promises) {
    try {
      ...
      console.error(`Failed to initialize codec ${meta.name}:`, e);
      throw e;
    }
  }
  if (!array_to_bytes) {
